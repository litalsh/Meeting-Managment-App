...
const MeetingContextProvider = (props) => {
  ...
  const changeListHandler = (id, type) => {
    ...
    if (type === 'heap') {
      ...
      setStackHistory([...stackHistory, { id: heap[taskIndex].id, topic: heap[taskIndex].topic, time: new Date().getTime }]);
  ...
  const downloadSummaryHandler = () => {
    ...
    element.download = "meetingSummary.pdf";
