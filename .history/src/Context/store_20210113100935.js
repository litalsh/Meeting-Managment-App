...
const MeetingContextProvider = (props) => {
  ...
  const taskRemoveHandler = (id, type) => {
    if (type === 'stack') {
      ...
      stack[0] !== undefined ? setCurrentTaskTimer(Math.floor((new Date().getTime() - stack[taskIndex-1].time)/1000)) : setCurrentTaskTimer(0);
    };
  ...
  const keyboardRemoveHandler = (e) => {
    if (e.ctrlKey && e.shiftKey && e.which === 88) {
      ...
      setStack(tempStack);
      setCurrentTaskTimer(stack[0] !== undefined ? Math.floor((new Date().getTime() - stack[stackLength-2 ].time)/1000) : 0);
