...
const MeetingContextProvider = (props) => {
  ...
  const [currentTaskTimer, setCurrentTaskTimer] = useState(stack[0] !== undefined ? new Date().getTime() - stack[stack.length - 1].time : null);
  ...
  const newStackHandler = (event) => {
    if (event.which === 13 && event.target.value !== '') {
      ...
      event.target.value = '';
      console.log(stack)
      setCurrentTaskTimer(stack[0] !== undefined ? new Date().getTime() - stack[stack.length - 1].time : null);
    };
  };
