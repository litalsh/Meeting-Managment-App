...
const MeetingContextProvider = (props) => {
  ...
  const stackText = htmlToText('<h3>Stack</h3><ul>' + stackHistory.join(' ') + '</ul>');
  let text = [];
  for(let i = 0; i < notes.length; i++){
    text.push(`<dt>${notes[i].topic}</dt>`);
    for(let j = 0; j < Notes[i].notes.length; j++){
      text.push(`<dd>${notes[i].notes[j]}</dd>`)
    }
  }
  const notesText = htmlToText('<h3>Notes</h3><dl>' + text.join(' ') + '</dl>');
  let text2 = [];
  heap.map((task)=>(text2.push(`<li>${task.topic}</li>`)));
  const heapText = htmlToText('<h3>Stack</h3><ul>' + text2.join(' ') + '</ul>');

  const downloadSummaryHandler = () => {
    console.log(stackText)
    const element = document.createElement("a");
    const file = new Blob([stackText, notesText, heapText], { type: "text/plain" });
